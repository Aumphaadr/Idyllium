...
async function main(): Promise<void> {
  ...
  const text = await runIdylliumInBrowser({
    ...
    files: {
      ...
      '/workspace/Lobster-Regular.ttf': {
        content: '',
        bytes: new Uint8Array(fs.readFileSync(path.join(process.cwd(), 'tests/fixtures/fonts/Lobster-Regular.ttf'))),
      },
    },
  });
  ...
  console.log('drawable origin: runtime API, Sprite/Text geometry and completions pass');
}
