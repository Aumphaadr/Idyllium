...
// Черновой страж раздела: все program-примеры widget colors lesson из манифеста
...
function main(): void {
  ...
  const examples = manifest.examples.filter((example: any) => (
    example.lessonPath === 'widgets/colors.html'
  ...
  assert(examples.length > 0, 'no widget colors lesson examples found in the manifest');
  ...
  console.log(`widget colors lesson draft spec: ${examples.length} examples compile`);
}
