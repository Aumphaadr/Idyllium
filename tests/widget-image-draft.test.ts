...
// Черновой страж раздела: все program-примеры widget image lesson из манифеста
...
function main(): void {
  ...
  const examples = manifest.examples.filter((example: any) => (
    example.lessonPath === 'widgets/image.html'
  ...
  assert(examples.length > 0, 'no widget image lesson examples found in the manifest');
  ...
  console.log(`widget image lesson draft spec: ${examples.length} examples compile`);
}
