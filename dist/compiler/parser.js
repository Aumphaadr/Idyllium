...
class Parser {
    ...
    parseClassBody(className) {
        ...
        while (!this.isAtEnd() && this.currentType() !== tokens_1.TokenType.RBRACE) {
            ...
            let isStatic = false;
            if (this.currentType() === tokens_1.TokenType.KW_STATIC) {
                isStatic = true;
                this.advance();
            }
            if (this.currentType() === tokens_1.TokenType.KW_CONSTRUCTOR) {
                if (isStatic) {
                    this.errors.addError(this.file, this.currentLine(), "constructors cannot be static");
                }
            ...
            if (this.currentType() === tokens_1.TokenType.KW_DESTRUCTOR) {
                if (isStatic) {
                    this.errors.addError(this.file, this.currentLine(), "destructors cannot be static");
                }
    ...
    parseClassFields(access, isStatic = false) {
    ...
    parseClassMethod(access, isStatic = false) {
    ...
    parseConstructor(className, access) {
        ...
        if (!this.expect(tokens_1.TokenType.LBRACE, "to open constructor body")) {
            return {
        ...
        let parentArgs = null;
        if (this.currentType() === tokens_1.TokenType.KW_PARENT &&
            this.peek(1).type === tokens_1.TokenType.LPAREN) {
            const parentLoc = this.here();
            this.advance(); // skip 'parent'
            this.advance(); // skip '('
            parentArgs = this.parseArgumentList();
            this.expect(tokens_1.TokenType.RPAREN, "after parent() arguments");
            this.expect(tokens_1.TokenType.SEMICOLON, "after parent() call");
        }
        const statements = [];
        while (!this.isAtEnd() && this.currentType() !== tokens_1.TokenType.RBRACE) {
        ...
        this.expect(tokens_1.TokenType.RBRACE, "to close constructor body");
        const body = {
            kind: 'Block',
            loc: blockLoc,
            statements,
    ...
    parsePrimary() {
        ...
        switch (tok.type) {
            ...
            case tokens_1.TokenType.KW_BOOL:
                ...
                break;
            case tokens_1.TokenType.KW_PARENT:
                this.errors.addError(this.file, this.currentLine(), "'parent()' can only be used as the first statement in a constructor");
                this.advance();
                return null;
            default:
                break;
