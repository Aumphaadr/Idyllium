...
class CodeGenerator {
    ...
    initFlagKey(className, moduleName) {
    ...
    genRawConstructor(className, hasParent, moduleName) {
    ...
    genAsyncInitializer(className, ctor, fields, hasParent, moduleName) {
        const params = ctor
    ...
    resolveParentConstructorParams(ctor, moduleName) {
        let ownerDecl = null;
        if (moduleName) {
    ...
    genVarDecl(decl) {
        ...
        if (decl.initializer !== null) {
            ...
            if (decl.varType.kind === 'ClassType') {
                const paramNames = this.methodParams.get(`${decl.varType.name}.__constructor__`) || [];
                const reorderedArgs = this.reorderArguments(decl.constructorArgs, paramNames);
                const args = reorderedArgs.map(a => this.genExpr(a.value)).join(', ');
    ...
    genAssignment(stmt) {
        ...
        if (stmt.operator === '=' && (0, resolved_types_1.isArrayLike)(targetType)) {
            const target = this.genAssignTarget(stmt.target);
            const value = this.genExpr(stmt.value);
            const fixed = targetType.tag === 'array' ? 'true' : 'false';
            const size = targetType.tag === 'array' ? targetType.size : -1;
    ...
    genBinary(expr) {
        ...
        if ((expr.operator === '==' || expr.operator === '!=') &&
            (0, resolved_types_1.isArrayLike)(leftType) && (0, resolved_types_1.isArrayLike)(rightType)) {
