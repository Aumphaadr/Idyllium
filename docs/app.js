...
(() => {
  ...
  function unzipStoredEntries(bytes) {
    ...
    while (offset + 4 <= bytes.length) {
      ...
      if ((flags & 8) !== 0) throw new Error("ZIP-архив с data descriptor пока не поддерживается");
  ...
  function registerMonacoIdyllium() {
    ...
    monaco.languages.registerCompletionItemProvider(MONACO_LANGUAGE_ID, {
      ...
      provideCompletionItems(model, position, context) {
        ...
        const items = projectCompletions(model.uri.path || currentFile, model.getValue(), request.requestOffset).filter((item) => {
