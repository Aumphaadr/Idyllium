// Загрузчик embed-юнитов: <idyllium-unit> на странице-хозяине.
//
//   <script src="https://aumphaadr.github.io/Idyllium/embed/idyllium-unit.js" async></script>
//   <idyllium-unit id="rect" on-solved="showBunny">
//     <script type="text/idyllium">…заготовка…</script>
//     <script type="application/json">{ "tests": […], "check": {…} }</script>
//   </idyllium-unit>
//
// Юнит живёт в iframe-песочнице с пустым origin: код ученика не видит ни
// страницы хозяина, ни хранилища Web IDE. Всё, что песочнице нельзя, делает
// этот скрипт на стороне хозяина: хранит черновик, подгоняет высоту, выдаёт
// DOM-события и зовёт функции хозяина ПО ИМЕНИ (жанр onclick, без eval).

const SOURCE = 'idyllium-unit';
// Обычный (не module) скрипт: ему не нужен CORS, и он работает на любом
// хостинге. Свой адрес узнаём по currentScript, а для type="module" — поиском.
const SELF = document.currentScript
  || Array.from(document.scripts).reverse().find((script) => /idyllium-unit\.js(?:[?#]|$)/u.test(script.src));
const BASE = new URL('./', SELF && SELF.src ? SELF.src : location.href).href; // …/Idyllium/embed/
const VERSION = '__IDYLLIUM_VERSION__';
...
function readConfig(element) {
  ...
  for (const script of element.querySelectorAll(':scope > script[type="text/idyllium"]')) {
    // Заготовка пишется как есть; срезаем только обрамляющие пустые строки.
  ...
  // Атрибуты поверх JSON: быстрые правки без перегенерации юнита.
  const editor = { ...(config.editor || {}) };
  const number = (name) => (element.hasAttribute(name) ? Number(element.getAttribute(name)) : undefined);
  if (number('rows') !== undefined) editor.rows = number('rows');
  if (number('console-rows') !== undefined) editor.consoleRows = number('console-rows');
  if (number('font-size') !== undefined) editor.fontSize = number('font-size');
...
/** Функция хозяина по имени или пути через точку: lms.report → window.lms.report. */
function resolveHostFunction(name) {
  ...
  for (const part of name.split('.')) {
    owner = value;
    value = owner == null ? undefined : owner[part];
  }
  return typeof value === 'function' ? value.bind(owner) : null;
}

class IdylliumUnit extends HTMLElement {
  constructor() {
    super();
    this.frame = null;
    this.config = null;
    this.pendingCode = new Map();
  ...
  connectedCallback() {
    ...
    // allow-popups — только ради ссылки «Работает на Idyllium»; программе ученика
    // открывать нечего: сетевых и url-библиотек в юните нет.
    frame.setAttribute('sandbox', 'allow-scripts allow-popups allow-popups-to-escape-sandbox');
    frame.setAttribute('title', config.title || 'Idyllium');
    frame.setAttribute('loading', 'lazy');
    // color-scheme:normal — иначе на сайте с color-scheme: dark браузер подложит
    // под кадр непрозрачный белый фон (схемы хозяина и кадра расходятся).
  ...
  readDraft() {
    try {
      ...
      return null; // хранилище может быть запрещено — юнит работает и без черновика
    }
  }
  ...
  writeDraft(code) {
    try {
      ...
      // нет хранилища — нет черновика
    }
  }

  send(message) {
    // У песочницы пустой origin — адресуем «*», но только своему кадру.
  ...
  setCode(code) { this.send({ type: 'command', name: 'setCode', code: String(code) }); }
  reset() { this.send({ type: 'command', name: 'reset' }); }
  run() { this.send({ type: 'command', name: 'run' }); }
  check() { this.send({ type: 'command', name: 'check' }); }
  /** Сменить тему на лету ('light' | 'dark' | 'auto') — для сайтов со своим переключателем темы. */
  setTheme(theme) { this.send({ type: 'command', name: 'setTheme', theme: String(theme) }); }
