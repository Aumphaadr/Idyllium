// Кадр embed-юнита: редактор, консоль, «Запустить / Остановить / Проверить».
// Живёт в iframe-песочнице (пустой origin): хранилища нет, наружу — только
// postMessage. Конфигурация приходит от загрузчика сообщением либо лежит в
// адресе после «#unit=» (форма «только iframe»).
//
// Ленивость: лёгкий редактор (textarea + наш подсветчик) работает сразу и
// ничего не весит; ядро Idyllium грузится при первом действии, Monaco — при
// первом фокусе в редакторе и подменяет лёгкий редактор на ходу.
import { KEYWORDS, BUILTIN_TYPES, CLASS_NAMES, QUALIFIED_TYPES, escapeHtml } from '../../web-ide/src/idyllium-highlight.js';
import { appendAnsiText } from '../../web-ide/src/ansi.js';
import { MONACO_LANGUAGE_ID, registerIdylliumGrammar, defineIdylliumThemes } from '../../web-ide/src/monaco-grammar.js';

const SOURCE = 'idyllium-unit';
const SITE = new URL('../', location.href).href; // …/Idyllium/
...
const UI = {
  ru: {
    ...
    checking: 'Проверка: тест {index} из {total}…', finished: 'Программа завершилась.',
    powered: 'Работает на Idyllium', broken: 'Юнит настроен с ошибкой:',
  ...
  en: {
    ...
    checking: 'Checking: test {index} of {total}…', finished: 'The program finished.',
    powered: 'Powered by Idyllium', broken: 'This unit is misconfigured:',
...
let config = null;        // нормализованный конфиг (после загрузки ядра) либо сырой «взгляд»
let rawConfig = null;
let activeTheme = 'auto';    // действующая тема: из конфига или из команды setTheme
let lang = 'ru';
let editor = null;
let state = 'idle';       // idle | running | checking
let stopProgram = null;
let pendingInput = null;  // { resolve, reject }
let attempts = 0;
let scriptPromise = null;   // ядро грузится один раз на кадр
let normalizedFor = null;   // для какого сырого конфига уже посчитан config
let monacoReady = null;     // Monaco + язык + автодополнение — один раз на кадр
let monacoPromise = null;   // подмена лёгкого редактора — один раз на монтирование

const setLabel = (button, text) => { button.querySelector('.label').textContent = text; };
...
/** Высота строки редактора — пропорция Web IDE: 25 px при кегле 16. */
const lineHeightFor = (fontSize) => Math.round(fontSize * 25 / 16);

function post(type, payload = {}) {
  // У песочницы пустой origin, адресат неизвестен заранее — только «*».
...
/** Взгляд на сырой конфиг до загрузки ядра: только то, что нужно первому кадру. */
function peek(raw) {
...
// ─── подсветка лёгкого редактора ───────────────────────────────────────────
// Повторяет правила Monarch-токенизатора из monaco-grammar.js (те же словари,
// те же решения «имя перед скобкой — функция», «после точки — член»), чтобы
// подмена лёгкого редактора на Monaco не меняла ни одного цвета.
const IDENT = '[A-Za-z_А-Яа-яЁё][A-Za-z0-9_А-Яа-яЁё]*';
const RE = {
  lineComment: /^\/\/[^\n]*/u,
  blockComment: /^\/\*[\s\S]*?(?:\*\/|$)/u,
  string: /^(?:"(?:\\.|[^"\\\n])*"?|'(?:\\.|[^'\\\n])*'?)/u,
...
function highlightLikeMonaco(source) {
  ...
  while (index < source.length) {
    ...
    if (match) { html += escapeHtml(match[0]); index += match[0].length; continue; }
    match = RE.lineComment.exec(rest) || RE.blockComment.exec(rest);
    if (match) { html += tok('comment', match[0]); index += match[0].length; afterDot = false; continue; }
    match = RE.string.exec(rest);
    if (match) { html += tok('string', match[0]); index += match[0].length; afterDot = false; continue; }
    match = RE.number.exec(rest);
    if (match) { html += tok('number', match[0]); index += match[0].length; afterDot = false; continue; }
...
function createLightEditor(host, value, onChange, onFirstFocus) {
  ...
  return {
    ...
    setValue: (text) => { input.value = text; paint(); sync(); },
    focus: () => input.focus(),
...
/** Ядро Idyllium: компилятор, рантайм и модуль embed. Нормализует конфиг при первой загрузке. */
function core() {
...
function activateMonaco() {
  ...
  monacoPromise = (async () => {
    ...
    // Monaco не приехал — лёгкий редактор остаётся рабочим, юнит жив.
...
async function prepareMonaco(api) {
  {
    ...
    monaco.languages.registerCompletionItemProvider(MONACO_LANGUAGE_ID, {
      ...
      provideCompletionItems(model, position) {
        // Автор юнита выключил автодополнение — молчим и на Ctrl+Пробел.
...
function swapInMonaco(api, monaco) {
  {
    ...
    // Автодополнение выключено автором — Ctrl+Пробел тоже молчит (без плашки «No suggestions»).
    ...
    instance.onDidChangeModelContent(() => { onCodeChange(); refreshMarkers(); });
    refreshMarkers();
    editor = {
      kind: 'monaco',
...
function clearOutput() { dom.output.textContent = ''; }
...
// Не форма: песочница без allow-forms молча глушит submit.
  ...
  await withCore(async (api) => {
    ...
    const run = await api.embed.runUnitProgram(api.createUnitRunner(), source, [], config, {
      interactive: {
        ...
        clear: clearOutput,
        onStopReady: (stop) => { stopProgram = stop; },
      },
    });
    stopProgram = null;
    closeInput();
...
async function checkSolution() {
  ...
  await withCore(async (api) => {
    ...
    if (report.verdict === 'solved') {
      ...
      lines.push(failure.input === ''
        ? text(lang, 'verdict.failed-noinput', { answer })
        : text(lang, 'verdict.failed-input', { input: failure.input, answer }));
    ...
    const detail = {
      ...
      firstFailure: report.firstFailure
        ? { input: report.firstFailure.input, answer: report.firstFailure.answer, reason: api.embed.reasonText(lang, report.firstFailure.reason) }
        : report.blocker ? { input: '', answer: '', reason: api.embed.reasonText(lang, report.blocker) } : null,
    };
...
// «Форматировать» — тот же выравниватель отступов, что у кнопки Web IDE. К
// недописанному коду терпим: выравнивает, что есть, и ничего не бросает.
function formatCode() {
...
// Подтверждение — вторым нажатием: window.confirm в песочнице без allow-modals
// молча возвращает false, и «Сбросить» не работал бы никогда.
let resetArmed = 0;
function resetCode(force = false) {
  ...
  if (!force && editor.getValue() !== starter && !resetArmed) {
    ...
    resetArmed = setTimeout(() => { resetArmed = 0; setLabel(dom.reset, t('reset')); }, 4000);
    return;
  }
...
window.addEventListener('message', (event) => {
  ...
  if (data.type === 'config') {
    ...
    else if (data.name === 'setCode' && editor) { editor.setValue(String(data.code ?? '')); onCodeChange(); }
    else if (data.name === 'getCode') post('code', { requestId: data.requestId, code: editor ? editor.getValue() : '' });
  }
});

const hashMatch = /[#&]unit=([A-Za-z0-9_-]+)/u.exec(location.hash);
if (hashMatch) {
  try {
...
// Загрузчик ждёт этого сигнала, чтобы прислать конфиг (форма «полная вставка»).
