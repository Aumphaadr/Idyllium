 ...
 */
export const IDYLLIUM_VERSION = '1.3.4';
...
function encodingCodepointToChar(codepoint: unknown, file: string, line: number): string {
...
function ensureTurtleField(state: RuntimeObjectState): TurtleFieldState {
...
function turtleCss(value: unknown): string {
  return value instanceof IdylliumColor ? value.toCss() : '#000000';
}

function rebuildTurtleFieldCommands(state: RuntimeObjectState): void {
...
function turtleAnimationSteps(state: RuntimeObjectState, speed: number, magnitude: number): number {
...
async function turtleFrame(state: RuntimeObjectState, file: string, line: number): Promise<void> {
  state.stopCheck?.(file, line);
  await new Promise((resolve) => setTimeout(resolve, TURTLE_FRAME_MS));
  state.stopCheck?.(file, line);
}

async function turtleTravel(
...
function normalizeTurtleHeading(value: number): number {
...
function initializeTurtleObject(obj: RuntimeObject, typeName: string, state: RuntimeObjectState): void {
...
function createTurtleModule(state: RuntimeObjectState): Record<string, unknown> {
  return {
...
function turtleSvg(field: TurtleFieldState): string {
    ...
    obj.create_from_image = contextFunction((sourceValue: unknown, file: string, line: number) => {
      if (!isRuntimeObject(sourceValue) || sourceValue.__idylliumType !== 'image.Static') {
        throw new IdylliumRuntimeError(
          file,
          line,
          `Bitmap.create_from_image() source must be image.Static, got '${runtimeTypeName(sourceValue)}'`,
        );
      }
...
function storedBitmap(obj: RuntimeObject, methodName: string, file: string, line: number): StoredBitmap {
  ...
  obj.collides_with = contextFunction((other: unknown, file: string, line: number) => {
    if (!isDrawableObject(other)) {
      throw new IdylliumRuntimeError(
        file,
        line,
        `${typeName}.collides_with() expects drawable object, got '${runtimeTypeName(other)}'`,
      );
    }
...
function channel(value: unknown, argumentName: string, file: string, line: number): number {
...
function opacity(value: unknown, argumentName: string, file: string, line: number): number {
