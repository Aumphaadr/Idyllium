...
function extractImportedModules(source: string): Set<string> {
...
export function tokenize(source: string): HighlightToken[] {
    ...
    function lastSignificantToken(): HighlightToken | null {
    ...
    function tokenBeforeDot(): HighlightToken | null {
        let dotFound = false;
        for (let i = tokens.length - 1; i >= 0; i--) {
    ...
    while (pos < len) {
        ...
        if (isIdentStart(ch)) {
            ...
            let category: TokenCategory = 'object';
