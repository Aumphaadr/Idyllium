...
import { compileIdyllium } from './index';
...
function assertCompiles(source: string, msg?: string): void {
...
function assertFails(source: string, expectedSubstring?: string, msg?: string): void {
    const result = compileIdyllium(source);
    if (result.success) {
        throw new Error(`${msg || 'Expected compilation to fail, but it succeeded'}`);
    }
    if (expectedSubstring) {
        const hasExpected = result.errors.some(e => 
            e.message.toLowerCase().includes(expectedSubstring.toLowerCase())
        );
        if (!hasExpected) {
...
function assertJsCodeContains(source: string, expectedSubstring: string, msg?: string): void {
    const result = compileIdyllium(source);
    if (!result.success) {
